...
const char* ssid = "YOUR_SSID"; // Input your wifi network name
const char* password = "YOUR_PASSSWORD"; // Input your wifi password
...
void waterr(){
  ...
  val= digitalRead(15);  // reading the coming signal from the soil sensor
  if(val == HIGH) // if water level is full then cut the relay 
  {
  digitalWrite(2,LOW); // low is to cut the relay
  }
  else
  {
  digitalWrite(2,HIGH); //high to continue proving signal and water supply
  }
